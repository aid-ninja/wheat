...
2. **Understand what's being claimed**: Break down the claim into its core assertions. For example, if p001 says "wheat-server.js is 133 lines, zero npm deps" — the assertions are: (a) 133 lines, (b) zero deps, (c) sufficient features.
...
4. **Create challenge claims**: For each substantive counter-argument found, create a claim with:
   ...
   - Set `conflicts_with` ONLY if the challenge directly contradicts the target claim's factual assertions. A risk or concern about something related is NOT a conflict — it's a separate risk claim.
...
5. **Run the compiler**:
   ...
   ```
   Report whether challenges created new conflicts and whether the compiler auto-resolved any (higher evidence tier wins).
